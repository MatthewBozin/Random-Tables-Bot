...
const modules = ["basic", "edit", "export", "tagging", "view"];
...
modules.forEach((c) => {
  fs.readdir(`./commands/${c}`, (err, files) => {
    ...
    console.log(
      `[Commandlogs] Loaded ${files.length} commands of modules ${c}`
    );
...
bot.on("ready", () => {
  ...
  let guildid = bot.guilds.cache.get("797207344282730496");
  let channelid = guildid.channels.cache.get("797207684347723777");
  channelid.send(`${bot.user.username} is back online!`);
});

bot.on("guildCreate", (guild) => {
  if (!fs.existsSync(`./memory/${guild.id}.json`)) {
    fs.writeFile(
      `./memory/${guild.id}.json`,
      `{"settings":{},"tables":{}}`,
      (err) => {
        if (err) console.log(err);
      }
